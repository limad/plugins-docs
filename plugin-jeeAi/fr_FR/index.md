...
- 🏠 Plus de **45 outils** couvrant l'ensemble de l'API Jeedom
...
| `search_market` | Cherche des plugins dans le catalogue Market Jeedom (mot-clé + catégorie optionnelle) |
| `get_market_plugin` | Détails d'un plugin Market (description, auteur, compatibilité, note) |
...
| Outil | Description |
|---|---|
| `list_jeeobjects` | Liste toutes les pièces avec leurs IDs |
...
| `get_jeeobject_state` | État de tous les équipements actifs d'une pièce (1 appel API) avec generic_type |
...
| `saveScriptDevice` | Créer ou mettre à jour un équipement Script (plugin Script requis) |
| `searchMarket` | Chercher des plugins dans le Market Jeedom |
| `getMarketPlugin` | Détails d'un plugin Market |
| `listViews` / `getView` / `addToViewZone` | Vues Jeedom |
| `listDesigns` / `getDesign` | Designs Jeedom |
...
| `jeedom://jeeobjects` | Liste de toutes les pièces |
...
| `jeedom://jeeobject/{id}` | État détaillé d'une pièce spécifique *(dynamique)* |

---

## Supervision temps réel — équipement McpSystem

Le plugin crée automatiquement un équipement **McpSystem** (singleton, `logicalId=McpSystem`) exposant 9 commandes de supervision :

| Commande | Type | Description |
|---|---|---|
| `daemon_state` | info/binary | Daemon actif (1) ou arrêté (0) — historisé |
| `client_connected` | info/binary | Client IA actuellement connecté |
| `last_client_name` | info/string | Nom de l'IA connectée (ex: "Cursor", "Claude Code") |
| `last_client_ip` | info/string | IP du dernier client |
| `last_client_access` | info/string | `local` / `LAN` / `externe` |
| `last_connection_date` | info/string | Horodatage de la dernière connexion |
| `connection_count_today` | info/numeric | Connexions du jour — historisé |
| `last_tool_called` | info/string | Dernier outil MCP utilisé |
| `refresh` | action/other | Rafraîchissement manuel |

Le nom du client est résolu automatiquement depuis `clientInfo.name` (message MCP `initialize`), avec fallback sur `resources/data/client_names.json` (table IP→nom éditable à chaud).
